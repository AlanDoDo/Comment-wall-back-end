...
exports.findWallPage = async (req, res) => {
    ...
    await dbModel.findWallPage(data.page, data.pagesize, data.type, data.label).then(async result => {
        for (let i = 0; i < result.length; i++) {
            // 查找相应wall的赞、举报、撤销数据
            // 喜欢
            result[i].like = await dbModel.feedbacksCount(result[i].id, 0);
            // 举报
            result[i].report = await dbModel.feedbacksCount(result[i].id, 1);
            // 要求撤销
            result[i].revoke = await dbModel.feedbacksCount(result[i].id, 2);
            // 是否点赞
